...
export default function TaskDetailPage() {
  ...
  useEffect(() => {
    ...
    const loadTaskDetail = async () => {
      ...
      try {
        ...
        setProgressInput(String(clampProgress(normalizedTask.progress)));
        ...
        const [goalResult, profilesResult] = await Promise.all([
  ...
  }, [form.description, form.name, form.note, task]);

  const hasExecutionChanges = useMemo(() => {
    if (!task) {
      return false;
    }
    return form.status !== normalizeTaskStatus(task.status) || form.progress !== clampProgress(task.progress);
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              onClick={() => {
                                ...
                                setProgressInput(String(clampProgress(task.progress)));
                                setIsEditingExecution(false);
                                setActionError(null);
                                setNotice(null);
                              }}
