...
function ScheduleDetailDialog({ item, canRegister, canManage, open, onOpenChange, onComplete, onItemChange }: { item: ScheduleBoardItem | null; canRegister: boolean; canManage: boolean; open: boolean; onOpenChange: (open: boolean) => void; onComplete: () => Promise<ScheduleBoardItem[] | null>; onItemChange: (item: ScheduleBoardItem) => void }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90vh] w-[calc(100vw-3rem)] max-w-[1440px] overflow-y-auto"><DialogHeader><div className="flex items-center justify-between gap-3 pr-10"><div className="flex items-center gap-2"><DialogTitle>{schedule.department?.name ?? "Lịch part-time"}</DialogTitle><StatusBadge status={schedule.status} automatic={schedule.finalizedAutomatically} /></div>{canManage && schedule.status === "open" ? <button type="button" onClick={() => setFinalizeConfirmOpen(true)} className="inline-flex h-9 items-center gap-2 rounded-lg bg-emerald-600 px-3 text-sm font-semibold text-white"><ClipboardCheck className="h-4 w-4" />Chốt lịch</button> : null}</div></DialogHeader>{canRemoveAnyEntry ? <p className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-900">Bạn là người tạo lịch nên có thể xóa trực tiếp mọi đăng ký, kể cả khi lịch đã chốt.</p> : null}<div className="rounded-xl border border-slate-200"><table className="w-full table-fixed text-left"><thead className="bg-slate-50"><tr>{getWeekDates(schedule.weekStart).map((date) => <th key={date} className="border-r border-slate-200 px-3 py-3 text-sm font-semibold capitalize last:border-r-0">{formatHanoiDate(date)}</th>)}</tr></thead><tbody><tr>{getWeekDates(schedule.weekStart).map((date) => <td key={date} className="align-top border-r border-t border-slate-200 p-2 last:border-r-0">{(["morning", "afternoon"] as ParttimeShift[]).map((shift) => { const people = entries.filter((entry) => entry.workDate === date && entry.shift === shift); const ownEntry = ownEntries.find((entry) => entry.workDate === date && entry.shift === shift) ?? null; const own = Boolean(ownEntry); return <div key={shift} className="mb-2 min-h-[66px] last:mb-0 rounded-lg bg-slate-50 p-2"><div className="flex items-center justify-between gap-1"><p className="text-xs font-semibold text-slate-500">{shiftLabel[shift]}</p>{canRegister ? schedule.status === "finalized" ? <FinalizedChangeButton entry={ownEntry} workDate={date} shift={shift} disabled={isBeforeWorkDateInHanoi(date)} pending={createChangeRequest.isPending} blocked={hasPendingChange} onSubmit={submitFinalizedChange} /> : <button type="button" disabled={locked || register.isPending || unregister.isPending} onClick={() => void toggleShift(date, shift)} className={cn("rounded px-1.5 py-1 text-[11px] font-semibold disabled:cursor-not-allowed disabled:opacity-50", own ? "bg-blue-600 text-white" : "border border-blue-200 bg-white text-blue-700")}>{own ? "Đã đăng ký" : "Đăng ký"}</button> : null}</div><div className="mt-2 min-h-5 space-y-1">{people.map((entry) => <div key={entry.id} className="flex items-center justify-between gap-2"><p className="truncate text-sm text-slate-700">{entry.profile?.name ?? "Không rõ"}</p>{canRemoveAnyEntry ? <button type="button" aria-label={`Xóa đăng ký của ${entry.profile?.name ?? "nhân sự"}`} disabled={unregister.isPending} onClick={() => setEntryToRemove(entry)} className="rounded p-1 text-rose-600 hover:bg-rose-50 disabled:opacity-50"><Trash2 className="h-3.5 w-3.5" /></button> : null}</div>)}</div></div>; })}</td>)}</tr></tbody></table></div>{schedule.status === "finalized" && canRegister && hasAnyPendingChange ? <p className="mt-3 text-xs font-medium text-amber-800">Bạn đang có yêu cầu thay đổi chờ Leader xử lý.</p> : null}{schedule.status === "finalized" && canManage ? <PendingChangeRequestList requests={pendingRequests.data} loading={pendingRequests.isLoading} onReview={(request, approve) => setReviewAction({ request, approve })} /> : null}<ConfirmDialog open={finalizeConfirmOpen} onOpenChange={setFinalizeConfirmOpen} title="Chốt lịch tuần này?" description="Sau khi chốt, nhân viên chỉ có thể thay đổi lịch bằng yêu cầu được Leader duyệt." pending={finalize.isPending} onConfirm={() => void finalizeSchedule()} /><ConfirmDialog open={Boolean(reviewAction)} onOpenChange={(isOpen) => !isOpen && setReviewAction(null)} title={reviewAction?.approve ? "Duyệt yêu cầu?" : "Từ chối yêu cầu?"} description="Thay đổi này sẽ được ghi nhận vào lịch part-time." pending={review.isPending} onConfirm={() => void reviewRequest()} /><ConfirmDialog open={Boolean(entryToRemove)} onOpenChange={(isOpen) => !isOpen && setEntryToRemove(null)} title="Xóa đăng ký ca này?" description={`Đăng ký của ${entryToRemove?.profile?.name ?? "nhân sự"} sẽ bị xóa khỏi lịch, kể cả khi lịch đã chốt.`} pending={unregister.isPending} onConfirm={() => void removeEntry()} /></DialogContent></Dialog>;
}
