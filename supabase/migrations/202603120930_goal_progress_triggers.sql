-- Auto-recalculate goal.progress when:
-- 1) task.progress / task.goal_id changes
-- 2) goal.progress changes (to propagate to parent goals)
-- Formula:
--   progress(goal) = (avg(progress of child tasks) + sum(progress of child goals)) / (A + 1)
-- where A = number of child goals of that goal.
...
-- Optional: run one-time backfill after deploying this migration.
-- This updates every goal based on current child tasks/goals data.
do $$
declare
